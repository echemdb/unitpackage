...
# Fixtures live in the project's root ``test/`` data tree (alongside
# ``test/loader_data`` and ``test/generated``). Anchor to the repo root the
# same way ``unitpackage.test.cli`` locates ``test/loader_data`` --- relative
# to the installed package --- rather than depending on an environment
# variable. This file sits at ``<repo root>/unitpackage/test/eln/``, so the
# repo root is four levels up.
FIXTURES_DIR = Path(__file__).resolve().parents[3] / "test" / "eln-fixtures"

CSV_DATA = b"t,E,j\n0.0,-0.103,43.01\n0.01,-0.102,51.41\n0.02,-0.101,42.89\n"

#: Column names of :data:`CSV_DATA`.
SAMPLE_COLUMNS = ["t", "E", "j"]

#: Canonical schema fields (with units) matching :data:`CSV_DATA`, shared by
#: the eLabFTW and Kadi4Mat tests when building ``unitpackage`` descriptors.
SAMPLE_FIELDS = [
...
#: Expected ``{name: unit}`` mapping after a descriptor round-trip.
SAMPLE_UNITS = {field["name"]: field["unit"] for field in SAMPLE_FIELDS}

#: Canonical frictionless datapackage descriptor (one CSV resource matching
#: :data:`CSV_DATA`). Shared by the eLabFTW and Kadi4Mat tests, which each wrap
#: it in their backend's own metadata storage format: eLabFTW nests it under a
#: ``"unitpackage"`` key in the entity's JSON ``metadata`` field, while Kadi4Mat
#: encodes it as a typed ``extras`` tree via ``_dict_to_kadi_extras``.
SAMPLE_DESCRIPTOR = {
    "resources": [
        {
            "name": "test_entry",
            ...
            "format": "csv",
            "mediatype": "text/csv",
            "schema": {"fields": SAMPLE_FIELDS},
            "metadata": {"echemdb": {"source": {"citationKey": "test_2026"}}},
        }
    ]
}


def get_versions(backend):
    """
...
def get_latest_version(backend):
    """
...
def get_csv_bytes(backend, version=None):
    """
    ...
    return (
        response.get("data", b"").encode("utf-8")
        if isinstance(response.get("data"), str)
        else b""
    )


def make_mock_uploads(backend, version=None):
    """
...
def make_mock_kadi_record(version, csv_bytes, backend="kadi"):
    ...
    def _download(file_id, file_path):  # pylint: disable=unused-argument
        # ``file_id`` is part of the real ``record.download_file`` signature
        # (callers pass it by keyword) but is unused by this fixture mock.
...
def assert_basic_entry(entry):
    """Assert *entry* is a well-formed Entry with the sample CSV data.

    Shared by the eLabFTW and Kadi4Mat ``fetch_entry`` tests.
    """
    assert entry is not None
    assert hasattr(entry, "df")
    assert list(entry.df.columns) == SAMPLE_COLUMNS
    assert len(entry.df) == 3


def assert_field_units(entry):
    """Assert *entry*'s field units match :data:`SAMPLE_UNITS`.

    Shared by the eLabFTW and Kadi4Mat round-trip / fixture tests.
    """
