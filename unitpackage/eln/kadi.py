...
class KadiClient(BaseELNClient):
    ...
    def __init__(self, host, pat, verify_ssl=True, base_path=None):
        r"""
        ...
        If the instance runs in a subpath the `base_path` can be set.
        ...
        # Always forward ``base_path`` (defaulting to ``""``). ``KadiManager``
        # only loads its config file when ``host``/``pat`` are absent; since we
        # always supply both, its internal ``config`` stays ``None``. Leaving
        # ``base_path`` unset then makes ``KadiManager`` dereference that
        # ``None`` config to look up a default value, raising
        # ``TypeError: 'NoneType' object is not subscriptable``. Passing ``""``
        # (i.e. no subpath) short-circuits that lookup.
        self._manager = KadiManager(
            host=host, pat=pat, verify=verify_ssl, base_path=base_path or ""
        )
    ...
    def from_env(cls):
        ...
        base_path = os.environ.get("KADI_BASE_PATH")
        ...
        return cls(host=host, pat=pat, base_path=base_path)
    ...
    def from_config(cls, profile=None):
        ...
        return cls(host=host, pat=pat, verify_ssl=verify_ssl, base_path=base_path)
    ...
    def info(self):
        ...
        """
        # Use a leading-slash (relative) endpoint so ``KadiManager`` prepends
        # the configured host *and* base path (for instances served from a
        # subpath); hand-building an absolute URL from ``self._host`` would
        # drop the base path.
        response = self._manager.make_request("/info", "get")
    ...
    def fetch_entry(self, entity_id):  # pylint: disable=too-many-locals
        ...
        resource = first_resource(descriptor_dict)
        target_csv_name = Path(resource.get("path", "")).name
        ...
        basename = Path(resource.get("path", "")).stem or f"record_{entity_id}"
    ...
    def fetch_entries(self, tags=None):
        ...
        # The Kadi API paginates search results (10 per page by default), so a
        # single request would silently drop every match beyond the first page.
        # Walk all pages, requesting the maximum page size the API allows.
        records = []
        page = 1
        while True:
            response = self._manager.search.search_resources(
                "record", page=page, per_page=100, **base_kwargs
            )
            ...
            if isinstance(data, dict):
                ...
                total_pages = data.get("_pagination", {}).get("total_pages", page)
            elif isinstance(data, list):
                page_records = data
                total_pages = page
            else:
        ...
        for rec in records:
            ...
            if rec_id is None:
                logger.warning("Skipping search result without a record id: %r", rec)
                continue
            items.append((rec_id, rec_title))

        return self._collect_entries(items, "record")
    ...
    def upload_entry(self, entry, title=None, tags=None):
        ...
        # ``create=True`` only creates a record when the identifier is free;
        # if one already exists, kadi-apy hands back that existing record
        # (``created`` is then False). Refuse to mutate a record we did not
        # create so an unrelated record is never clobbered. We raise before
        # uploading anything, so there is nothing to roll back.
        if not getattr(record, "created", True):
            raise ValueError(
                f"A Kadi4Mat record with identifier '{identifier}' already "
                "exists. Refusing to modify it; choose a different title or "
        ...
        try:
            ...
            # A record without the ``unitpackage`` metadatum can never be
            # fetched back, so leaving it behind would orphan it. Roll back the
            # partially created record before re-raising.
            logger.warning(
                "Upload failed, deleting partially created record %s", entity_id
            )
            try:
                record.delete()
            except Exception:  # pylint: disable=broad-except
                logger.error(
                    "Failed to clean up record %s after upload error", entity_id
                )
            raise
