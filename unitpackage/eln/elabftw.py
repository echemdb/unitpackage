...
Provides the :class:`ElabFTWClient` class for fetching items (eLabFTW
resources) from an eLabFTW instance as unitpackage entries, and for uploading
entries back to eLabFTW.

.. note::

    Only the eLabFTW ``items`` entity type (resources) is supported at the
    moment. eLabFTW *experiments* are not yet supported.
...
class ElabFTWClient(BaseELNClient):
    ...
    Provides methods to fetch items (eLabFTW resources) as unitpackage entries,
    and to upload entries back to eLabFTW.

    .. note::

        Only the eLabFTW ``items`` entity type is supported at the moment;
        eLabFTW *experiments* are not yet supported.
    ...
    def __init__(self, host, api_key, entity_type="items", verify_ssl=True):
        ...
        Only the ``"items"`` entity type (eLabFTW resources) is supported at
        the moment; eLabFTW *experiments* are not yet supported, so
        ``entity_type`` must be ``"items"`` (the default) and any other value
        raises ``ValueError``.
    ...
    def info(self):
        ...
        # ``get_info()`` returns an ``Info`` model object, not a dict; normalise
        # it so callers (and the CLI's ``.get()`` lookups) get a plain dict.
    ...
    def fetch_entries(self, tags=None):
        ...
        items = [(entity.id, entity.title) for entity in up_entities]
        return self._collect_entries(items, entity_type)
    ...
    def upload_entry(
        ...
        # eLabFTW returns the new entity's URL in the Location header; the ID
        # is the last path segment. Without it we cannot identify (or later
        # clean up) the created item, so fail with a clear error.
        location = headers.get("Location")
        if not location:
            raise ValueError(
                "eLabFTW did not return a 'Location' header for the created "
                f"{entity_type}; cannot determine the new entity ID."
            )
        try:
            entity_id = int(location.rstrip("/").split("/").pop())
        except ValueError as exc:
            raise ValueError(
                f"Could not parse entity ID from Location header '{location}'."
            ) from exc
